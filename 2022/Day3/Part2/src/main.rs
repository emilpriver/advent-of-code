...
fn main() {
    ...
    let contents_array = contents.lines().into_iter().chunks(3);

    let mut equal_chars: Vec<char> = vec![];
    let mut total_points: usize = 0;

    for line in &contents_array {
        let line_into_vec = line.into_iter();
        let [first_line, second_line, third_line] = line_into_vec[0..3]  else { panic!("rip") };
        ...
        let mut third_part_array: Vec<char> = third_line.chars().into_iter().unique().collect();
